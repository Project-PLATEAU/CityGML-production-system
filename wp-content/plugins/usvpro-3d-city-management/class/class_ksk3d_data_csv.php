<?php
class ksk3d_data_csv extends ksk3d_view_list{
  static $tbl = KSK3D_TABLE_DATA;
  static $tbl_ref = [
    'csv2db_set_attrib'=>array(
  ...
  static function view2(){
    ...
    } else if (isset($_POST["submit"]["csv2db_set_attrib"])) {
      return static::csv2db_set_attrib();
    } else if (isset($_POST["submit"]["csv2DB_check2"])) {
      return static::csv2DB_check2();
    } else if (isset($_POST["submit"]["csv2DB_exec"])) {
      return static::csv2DB_exec();
    } else if (isset($_POST["submit"]["csv2DB_bgexec"])) {
      return static::csv2DB_bgexec();
    } else {
      return static::disp();
    }
  }
  ...
  static $button_value = [
    ...
    'csv2db_set_attrib' => '属性項目の設定',
    'csv2DB_check2' => '内部データセットへ変換確認',
    'csv2DB_exec' => '内部データセットへ変換実行',
    'csv2DB_bgexec' => 'バックグランドで実行',
   ];
  ...
  static function csv2db_set_attrib(){
    $page = 'csv2db_set_attrib';
    $tab = 'csv2db_set_attrib';
    ...
    $sql = "SELECT file_id,file_path,file_name,file_format FROM {$tbl_name} WHERE id = %d;";
    $prepared = $wpdb->prepare($sql, $form_id);
    $result = $wpdb->get_row($prepared ,ARRAY_A);
    $zip_pathinfo = ksk3d_functions_zip::pathinfo($result['file_id']);
    if (! is_file($zip_pathinfo['fullpath'])){
      $zip_pathinfo['fullpath'] = ksk3d_fileid_zip_Compress($result['file_id']);
    }
    $file1 = ksk3d_zip_extractTo1($zip_pathinfo['fullpath']);
...
;
    ...
    if (isset($_POST["set_attrib"])) {
      ...
      $result = ksk3d_functions_csv::test($file1);
...
;
  ...
  static function csv2DB_exec(){
    ...
    $result = ksk3d_conv($form_id ,"" ,"ksk3d_functions_csv::internal" ,array("内部データセット","内部データセット(属性)") ,$set_attrib);
    $text .= $result[1];
  ...
  static function csv2DB_bgexec(){
    ...
    $text .= ksk3d_fn_proc::registration(
      ...
      array(
        $form_id,
        "",
        "ksk3d_functions_csv::internal",
        array("内部データセット","内部データセット(属性)"),
        $set_attrib
      ),
      5,
      1
